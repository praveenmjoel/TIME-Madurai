/**
 * TIME Madurai – AIMCAT Data Extractor Bookmarklet (v5)
 *
 * How to install:
...
(async function () {
  ...
  const STORE_KEY = 'aimcat_extract_v5';
  ...
  async function extractSubarea() {
    ...
    // ── Find accordion container first ────────────────────────────────────────
    // "QA" appears in both the page-level nav tab AND the accordion header.
    // Anchor on VA/RC (unique to accordion), walk up until we find an ancestor
    // that also contains DI/LR and QA headers — that's the accordion root.
    // Then scope all three section searches inside it.
    const vaHeader = [...document.querySelectorAll('*')].find(e =>
      e.textContent.trim() === 'VA/RC' && e.children.length <= 2
    );
    let accordionScope = document;
    if (vaHeader) {
      let el = vaHeader.parentElement;
      for (let depth = 0; depth < 10; depth++) {
        if (!el || el === document.body) break;
        const kids = [...el.querySelectorAll('*')];
        const hasDILR = kids.some(c => c.textContent.trim() === 'DI/LR' && c.children.length <= 2);
        const hasQA   = kids.some(c => c.textContent.trim() === 'QA'    && c.children.length <= 2);
        if (hasDILR && hasQA) { accordionScope = el; break; }
        el = el.parentElement;
      }
    }

    // ── Expand each section within the scoped accordion container ─────────────
    for (const name of sectionNames) {
      const el = [...accordionScope.querySelectorAll('*')].find(e =>
